...
首页“数据清理”可以选择保留最近 5、10 或 20 期。系统先调用只读预览，列出将删除的
期次、文章、原始记录和运行日志；确认后才执行不可恢复的清理。正在抓取或分析时禁止清理。

清理会保留仍被任一期次引用的共享文章，并删除未进入期次的原始缓存、孤立文章及其分析、
孤立作者/标签/事件聚类。每个来源最多保留最近 20 次抓取记录，每篇保留最近 5 次分析尝试。
该功能不会自动执行，以免后台意外删除数据。
...
对规范化后的文章使用阿里云百炼 `qwen3.7-flash-2026-07-15` 运行版本化结构分析：
...
评测集的说明见 [`backend/docs/ai-analysis.md`](backend/docs/ai-analysis.md)。深度分析会先运行
统一证据研究工作流，从 PDF、HTML、JSON 或纯文本补齐可引用资料；工具边界、失败状态和
后续 Agent 化条件见
[`backend/docs/evidence-research-workflow.md`](backend/docs/evidence-research-workflow.md)。
