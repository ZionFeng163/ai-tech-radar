...
def test_angle_rejects_unnecessary_foreign_cultural_example() -> None:
    angle_set = _angle_set(
        _angle(
            label="记得却没用上",
    ...
    try:
        _validate_angle_set(
            ...
            {
                "title": "Keep It InMind",
                "source_excerpt": "A tree-nut allergy should affect a food recommendation.",
            },
        )
    except ValueError as exc:
        assert "外国文化例子" in str(exc)
        assert "马卡龙" in str(exc)
    else:
        raise AssertionError("non-essential cultural references must be generalized")
