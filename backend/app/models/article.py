...
class Article(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    heat_score: Mapped[float | None] = mapped_column(Float, index=True)
    signal_type: Mapped[str | None] = mapped_column(String(30), index=True)
    technical_overview: Mapped[str | None] = mapped_column(Text)
    novelty_summary: Mapped[str | None] = mapped_column(Text)
    heat_reasons: Mapped[list[str]] = mapped_column(JSONB, default=list, nullable=False)
