...
默认的 `deterministic` Provider 无需网络或密钥，适合本地开发、CI 和数据链路验收。它遵守同一
Schema，但不是生产内容质量的替代品。生产环境可将 `config/analysis.json` 改为：
...
Provider 使用 Responses API 的 `text.format` JSON Schema 严格输出。密钥只从
`OPENAI_API_KEY`（或 `api_key_env` 指定的变量）读取，不进入配置、请求审计或数据库。
